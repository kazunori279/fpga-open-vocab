...
```
uv run host/m7.py   --bitstream rtl/bitstreams/m11/gemm_top.hex \
                    --wide      rtl/bitstreams/m11/gemm_top_wide.hex
uv run host/demo.py --bitstream rtl/bitstreams/m11/gemm_top_wide.hex
```

| | link config | measured on hardware |
|---|---|---|
| [`m11/`](m11/README.txt) | `gemm_top` = A (narrow), `gemm_top_wide` = C | **current.** m7 ladder PASS both, config C 845 ms at 75.0 MHz |
| [`m10/`](m10/README.txt) | same | previous known-good, kept as the fallback and as the seed evidence above |
...
anyway** — 75 MHz `link_clk` against a 58–65 MHz report, which has been true of
