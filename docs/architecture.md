...
| `tools/synth_pairs.py`, `tools/synth_sheet.py` | a contrast pair with no camera and no object: crop a COCO instance box out of val2017, have `gemini-3-pro-image` edit the crop into both states, then screen every pair **blind** — sides shown as A\|B, alternating, judged on which side holds the positive state — before anything is encoded. Survivors go in a `keep.txt` that `probe_bisect.py --keep` reads. Screens the **teacher** only: a set of different scenes asks whether any open book beats any closed one, which is not the question a bench asks, and the student answers it upside down |
