...
| `host/cue.py` | the bench as an instrument: speaks scene changes on a schedule and records which frame each cue landed on. Its window placement is why 20-frame enrolment beats 1. `--frame-check` and `--preview N` keep a PNG of what the camera sees, and with `--enrol` a picture of each enrolment window is kept beside the log unasked — the 2026-08-17 run is why |
