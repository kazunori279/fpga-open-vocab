...
**The void was filled by the first run that tested it prospectively, and the guard is one-sided as a result.** 11:26 on 2026-08-17 is the first bench whose references the *board* built from two visits rather than a log replayed afterwards. It read 1.12, on the reject side, and scored 92.5% held out, the best in the project, while the board printed `THE CLASSES OVERLAP` and told the operator to start over. Eleven runs now: above 2.6 there are three and they scored 91.7%, 96.7% and 100.0%; the eight below it run from 92.5% down to 47.5%. **So the bar is a sufficient condition and not a necessary one**, `FGX_ENROL_SNR` is 2.6 rather than 2.0 because 2.64 is the lowest ratio that has ever certified anything, and below the bar the board prints the numbers and no advice. Three quantities measurable at enrolment have now failed the same way — `sep`, ratio(1), ratio(2) — and the reason is structural: what decides a run is where the object lands on visits that have not happened yet. 11:26 states it exactly, with enrolment visits 0.05 apart and held-out visits 1.67 and 1.72 apart: **the reference pair was tiny and pointed the right way, which is all classification needs.** Every held-out percentage in this section is the board's live figure, not `score_cue.py`'s one-visit replay; the two disagree by up to 83 points on the same log, and `bench/README.md` lists both for all twenty-five benches, which are archived under `bench/cue/`. Enrolling from two visits is *not*, separately, an accuracy win: a leave-one-visit-out replay (`tools/probe_multivisit.py`) puts two-visit references within a point of one-visit ones on the two clean benches and 18 points *worse* on one 08-16 run. The second visit is here to make the ratio measurable, not to make the classifier better.
...
**Nine benches now vote, and they say retuning the constant is worth about five points rather than thirty.** `tools/probe_presence.py` replays the rule over every archived bench in `bench/cue/`. Three of the nine are genuinely inverted — AUC 0.274, 0.276, 0.319, the empty desk sitting *nearer* the references than the objects do — and no radius touches those. On the other six the geometry points the way the rule assumes, four of them at AUC 0.904 to 0.956, and a radius fitted to that bench scores 72.6% to 94.9% balanced where the shipped constant scores 50%. **But it is not the same radius**: the per-bench optimum runs 0.55 to 3.60 in absolute distance and 0.50 to 3.90 in `sep`. Leave one bench out, fit the radius on the other eight, score it on the one it has not seen, and the three candidate units come out within four points of each other — `sep` 64.1%, the enrolment spread `scat` 67.1%, raw distance 67.8% — against **59.2% for `FGX_ABSENT_TRIP = 2.0 sep` as it ships**. So the sweep is not blocked, and it is also not the fix: it buys five to eight points, and the reason is the same structural fact the enrolment guard ran into three times. The right threshold is a property of the bench, and nothing measured at enrolment predicts it. What #18 needs next is evidence the distance does not carry, not a fifth constant fitted to these nine.

**Two more benches with the empty rotation on, at 11:26 and 11:44, say the radius is not what is wrong.** They held 7/90 (7.8%) and 0/90 (0.0%), and the reason is visible one line above the verdict: the empty desk's mean distance to the nearest reference was **0.87 and 1.06 sep against the class frames' 0.70 and 0.44**. The desk is *further* from the references than the objects are, which is the ordering the rule cuts on — so no radius exists that admits the classes and rejects the desk, and `an opened book` took 78 and 86 of the 90 empty frames. That is the same inversion as 07:33, which makes it three benches and therefore the scene rather than an accident: with the background frozen on an empty desk, an opened book lying in front of the camera sits **between** a closed book and nothing at all. The open question for this stage is that geometry, not `FGX_ABSENT_TRIP`.
