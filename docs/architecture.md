...
**Ten benches now vote, and they say there is no radius worth shipping.** `tools/probe_presence.py` replays the rule over every archived bench in `bench/cue/`. Three of the ten are genuinely inverted — AUC 0.274, 0.276, 0.319, the empty desk sitting *nearer* the references than the objects do — and no radius touches those. On the other seven the geometry points the way the rule assumes, five of them at AUC 0.904 to 0.956, and a radius fitted to that bench scores 72.6% to 94.9% balanced where the shipped constant is at or within two points of the 50% floor on five of the seven. **But it is not the same radius**: the per-bench optimum runs 0.55 to 3.60 in absolute distance and 0.50 to 3.90 in `sep`. Leave one bench out, fit the radius on the other nine, score it on the one it has not seen, and the honest figure is what comes back:
