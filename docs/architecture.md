...
**Five: enrolment, because neither axis carries a threshold.** The operator **shows** the board each scene — press `'1'`..`'6'` for the class named by that query — and it keeps what it saw: `FGX_ENROL_N = 20` frames of `c[]`, averaged (`m9.c:1583-1608`). That is the design decision the measurements pushed hardest on. The ordering was already right and the boundary was not at zero — AUC 1.000 with 79.4% correct at a margin of zero, because the right cut was −3.79 — so the *boundary* is the thing to learn and the ranking is not. There used to be a `'0'` for the empty scene as well; [#18](https://github.com/kazunori279/fpga-open-vocab/issues/18) deleted it, for the reason the presence subsection below gives.
...
The third was added on 2026-08-17 for a failure the other two are structurally unable to see, and it is the only one whose scale does not come from `sep`. **`sep` is the unit every other distance here is quoted in**, so a collapsed pair does not read as small — it inflates everything measured against it, which is how the 08-17 08:55 run put both references 26 *sep* from the origin with the origin guard quiet and 0.20 between them. The one scale in the problem that `sep` does not set is the enrolment frames' own spread: 20 frames are already being averaged into a reference, so a second accumulator (`Σ cz²` beside `Σ cz`) gives the RMS distance of one frame from its window mean for free. A frame lands nearer the wrong reference once that noise exceeds half the gap, hence **`sep < FGX_ENROL_SNR × scatter`, `FGX_ENROL_SNR = 2.0`, is the two classes being one blob** — printed as `nearest pair 0.20 apart, scatter 2.35 (0.08x)` and followed by **THE CLASSES OVERLAP**. Measured on all six cue benches before the constant was written: the runs that worked read 15.59, 4.07 and 3.62 and the runs that did not read 0.89, 0.51 and 0.08, so 2.0 sits in a gap four times wide rather than on a fitted edge. `tools/probe_reject.py` prints the same figures off a log, and agrees with the board to the printed digit.

**This is a floor and not a predictor, and the first two runs to clear it said so.** 09:18 and 09:33 on 2026-08-17 read 2.85 and 2.75 and scored 91.7% and 59.2% held out. The scatter is measured inside *one* enrolment window — one visit, the operator holding still — and what decides the run is where the same object lands when it is staged *again*, which nothing can see at the moment the reference is taken because there has only been one visit. On 09:33 the opened book's second visit centred 0.13 sep from the *closed* book's reference and 0.87 from its own, so thirty frames went to the other class and the board was right by its own rule each time. The quantity that does order all eight benches is the worst held-out visit's margin (+0.86, +0.58, +0.55 for the runs above 90%, negative for every run below 77%) and it is a diagnostic, not a guard, for the same reason. What it points at is enrolling a class from more than one visit, so its reference sits in the middle of its own spread instead of at one edge — untested, and testable offline against the existing logs.
